...
ChordBox = function (paper, x, y, width, height) {
    ...
    this.width = (!width) ? 65 : width;
    this.height = (!height) ? 65 : height;
    this.num_strings = 6;
    this.num_frets = 5;
...
ChordBox.prototype.draw = function () {
    ...
    for (var i = 0; i < this.num_strings; i++) {
        this.paper.verLine(this.x + string_spacing * i, this.y, this.y + this.height)
    }

    // Draw frets
    for (var i = 0; i < this.num_frets + 1; i++) {
        var position = chords[chord_name]["positions"][5 - i]
        var path = this.paper.horLine(this.x, this.y + fret_spacing * i, this.x + this.width);
        if (position == 'x') {
            path.attr({stroke: 'red', 'stroke-dasharray': '--'});
        }
    }

    // Draw chord name
    this.paper.text(this.x + this.width / 2 + string_spacing / 3, this.y - fret_spacing, chord_name).attr({
        "font-size": this.name_size,
        'stroke-width': 4,
        "fill": "green"
    })
    ...
    if (capo != 1) {
        this.paper.rect(this.x, this.y - fret_spacing * 3 / 2, fret_spacing, fret_spacing).attr({"fill": "blue", "stroke":"none"})
        //this.paper.circle(this.x + string_spacing * 0.5, this.y - fret_spacing, fret_spacing / 3).attr("fill", "black")
        this.paper.text(this.x + string_spacing * 3/ 7, this.y - fret_spacing, capo).attr("fill", "white")
    }
    ...
    for (var i = 0; i < 6; i++) {
        ...
        if (!(position == 'x' || position == '0')) {
            ...
            this.paper.circle(this.x + string_spacing * (fret - capo + 1) - fret_spacing / 2, this.y + fret_spacing * (5 - i), fret_spacing / 3).attr("fill", "green")
            this.paper.text(this.x + string_spacing * (fret - capo + 1) - fret_spacing / 2, this.y + fret_spacing * (5 - i), finger).attr("fill", "white")
        }
    }
    return
}
